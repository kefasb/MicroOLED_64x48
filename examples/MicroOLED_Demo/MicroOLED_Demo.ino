...
//////////////////////////////////
SpiCommunicationOled communicationSpi = SpiCommunicationOled(PIN_DC, PIN_CS);
//I2cCommunicationOled communicationI2c = I2cCommunicationOled(); // Example I2C declaration
MicroOLED oled(communicationSpi, PIN_RESET);

void setup() {
    oled.begin();    // Initialize the OLED
    oled.clear(OledClearMode::ALL); // Clear the display's internal memory
    oled.display();  // Display what's in the buffer (splashscreen)
    delay(1000);     // Delay 1000 ms
    oled.clear(OledClearMode::PAGE); // Clear the buffer.
...
void lineExample() {
    ...
    for (int i = 0; i < 3; i++) {
        ...
        for (int deg = 0; deg < 360; deg += 15) {
            ...
            oled.line(middleX, middleY, middleX + xEnd, middleY + yEnd,
                    OledDrawColor::BLACK, OledDrawMode::NORM);
            oled.display();
            delay(10);
        }
    }
...
void shapeExample() {
    ...
    while ((ball_X - ball_rad > 1)
        ...
        // Draw the Pong Field
        oled.clear(OledClearMode::PAGE);  // Clear the page
...
void textExamples() {
    ...
    oled.clear(OledClearMode::PAGE);     // Clear the screen
    ...
    // analog pins.
    for (int i = 0; i < 25; i++) {
        oled.clear(OledClearMode::PAGE);            // Clear the display
        oled.setCursor(0, 0);        // Set cursor to top-left
